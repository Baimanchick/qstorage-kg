        ...
        <LoaderData isLoading={incomingItemLoading} data={outGoingItem}>
          <div className={cls.navigation__info}>
            ...
            <Flex className={cls.incoming_header} justify="space-between" align="center">
              <Flex align="center" gap={10} className={cls.incoming_header_title}>
                <h2>Уходдуе товарлары №{outGoingItem?.act}</h2>

                <span className={cls.incoming_date}>
                  {dayjs(outGoingItem?.date).format('DD MMMM YYYY HH:mm')}
                </span>
                <Tag color={getTagColor(outGoingItem?.status ?? '')}>
                  {checkStatus(outGoingItem?.status ?? '')}
                </Tag>
              </Flex>
              ...
              <Flex gap={10} align="center" className={cls.btns}>
                <Button
                  ...
                  className={cls.edit}
                >
                  <Link href={`/products/outgoing/edit/${outGoingItem?.id}/`}>Өзгөртүү</Link>
                </Button>
              </Flex>
            </Flex>
          </div>
          ...
          <Flex className={cls.incoming_info} vertical>
            <Flex className={cls.incoming_info__block}>
              <h3 className={cls.details_title}>Уход боюнча маалымат</h3>
            </Flex>
            ...
            <Flex className={cls.details_items} vertical gap={10}>
              {outGoingItem?.items.map((product) => (
                <Flex className={cls.details_item} key={product.product.slug} gap={10}>
                  <Image
                    src={
                      product.product.first_image?.image
                        ? `${NEXT_PUBLIC_COMPANY_BASE_URL}${product.product.first_image.image}`
                        : NoPhoto.src
                    }
                    alt={product.product_title}
                    width={150}
                    height={150}
                    className={cls.incoming_product_image}
                  />
                  ...
                  <Flex className={cls.product_typography} vertical>
                    ...
                    <div className={cls.act}>
                      <span className={cls.heading}>Жарактуулук мөөнөтү:</span>{' '}
                    ...
                    <div>
                      <span className={cls.heading}>Жалпы сумма:</span> {product.purchase_price}
                    </div>

                    <Image
                      src={`${NEXT_PUBLIC_COMPANY_BASE_URL}${product.product.barcode}`}
            ...
            <Flex className={cls.total} vertical gap={5}>
              <span className={cls.total_quantity}>
                <span className={cls.heading}>Уходтун жалпы саны:</span>{' '}
                {outGoingItem?.total_quantity}
              </span>
              <span className={cls.total_quantity}>
                <span className={cls.heading}>Уходтун жалпы суммасы:</span>{' '}
                {outGoingItem?.total_price}
              </span>
            </Flex>
          </Flex>

          <Flex className={cls.docs} vertical>
            <h1 className={cls.details_title}>Уход боюнча документтер</h1>
